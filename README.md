...
- **A Game of Thrones**: a separate build of this mod, adapted for the _A Game of Thrones_ total conversion, is published on its own. It swaps in AGOT's Settle Wilderness directive, adds Westeros conditions (Is Ironborn, follows the Faith of the Seven, the Old Gods, or R'hllor, and holds land in the Seven Kingdoms), retunes the Military Strength thresholds for AGOT's larger armies, and drops the nomad section AGOT does not use. Play that version alongside AGOT instead of this one, and load it after AGOT in your playset so an exempt vassal's directive icon still dims (both mods redefine the directive-display text functions, so whichever loads last wins; load it before AGOT and everything works except the dimming).
