...
**A preset is a starting point, not a take-it-or-leave-it.** Change any of its rules and the whole thing becomes yours, exactly as it was, plus your change — so suiting one to your realm costs a click rather than a rebuild. Picking Custom outright starts you from nothing instead. Your own rules are kept aside while a preset is loaded, so trying one never costs you your waterfall.
...
4. **Exempt individuals**: right-click a vassal's portrait → Vassal section → **"Exempt from Directive Automation"**. Exempt vassals show their directive icon **dimmed grey** everywhere in the UI and are skipped entirely; manage them by hand with the vanilla _Give Vassal Directive_ interaction. Undo with **"Include in Directive Automation"**.
...
⚠️ The rules **overwrite manual directive assignments on non-exempt vassals** on their next run. Exemption is how you protect a manual choice. Directives you assigned by hand are otherwise never touched — the mod only ever clears a directive it still recognises as its own.
...
- **Other mods**: the only vanilla file override is `common/customizable_localization/00_vassal_custom_loc.txt` (adds the greyed exemption marker). Any mod overriding the same file will conflict on that cosmetic feature only. The panel is added as a standalone widget and overrides no vanilla GUI file.
...
gui/leo_mvd_texticons.gui                                grey and inline directive icons
...
- **The panel is generated** (`tools/gen_panel.sh`). The editor is the same widgets repeated per node, differing only in the variable they bind to, and GUI has no way to factor that out — `blockoverride` cannot parameterise a variable name inside a binding. Six priorities plus the nomads' three come to ~25,000 lines. **Edit the generator, not the files it writes.**
