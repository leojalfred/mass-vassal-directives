...
Without Roads to Power or All Under Heaven, confirm instead that the gated conditions and directives are absent. With All Under Heaven but not Roads to Power, confirm the administrative directives and the Administrative Government condition are all present and only Governor Theme is missing. For AGOT, load `dist/agot` after AGOT and repeat: the Westeros conditions appear, Settle the Wilderness assigns, and the exempt dimming works (it depends on load order).
